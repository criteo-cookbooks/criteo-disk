...
load_current_value do |new_resource|
  all_infos = ::CriteoDisk::Utils.scan_existing(node, new_resource.disk)
  ...
  if part_infos
    ...
    if new_resource.size.eql?('ALL')
      if ::CriteoDisk::Utils.close_to(part_infos['size'], all_infos['meta']['size'])
        size 'ALL'
      else
      ...
      size "#{::CriteoDisk::Utils.convert_size(part_infos['size'], unit, all_infos['meta']['logical_block_size'])}#{unit}"
    end
  ...
  converge_if_changed(:part_name, :size, :file_system) do
    unless current_resource.nil?
      ...
      mount current_resource.mount_point do
        device lazy { "#{disk}#{::CriteoDisk::Utils.find_part(node, disk, current_resource.part_name)}" }
        action [:umount, :disable]
        not_if { current_resource.mount_point == 'None' }
      end
      ruby_block "Deleting partition #{current_resource.part_name}" do
        block do
          ::CriteoDisk::Utils.destroy_part(node, disk, current_resource.part_name)
        end
        action :run
      end
    end
    ...
    parted_disk disk do
      part_type part_name
      part_start lazy { ::CriteoDisk::Utils.find_limits(node, disk, size)[0].to_s }
      part_end lazy { ::CriteoDisk::Utils.find_limits(node, disk, size)[1].to_s }
      flag_name flag
      action todo
    end
    ...
    filesystem part_name do
      fstype file_system
      device lazy { "#{disk}#{::CriteoDisk::Utils.find_part(node, disk, part_name)}" }
      mount mount_point
      options mount_opts
